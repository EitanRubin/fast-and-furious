...
export class Step1Component {
  private apiService = inject(MockApiService);
  private wizardState = inject(WizardStateService);

  applications = this.apiService.getApplicationNames();
  selectedApp = signal<string | null>(null);

  getHostCount(app: string): number {
    return this.apiService.getHostsForApplication(app).length;
  }
  ...
  proceed() {
    ...
    if (app) {
      const hosts = this.apiService.getHostsForApplication(app);
